# Configuration Onboarding Doc — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Créer `docs/configuration.md` — guide d'onboarding séquentiel + référence exhaustive pour configurer lean-jira de zéro jusqu'au premier rapport.

**Architecture:** Fichier Markdown unique organisé en 7 sections : flux d'onboarding dans l'ordre (sections 1–4), référence champ par champ (section 5), cas avancés (section 6), troubleshooting (section 7). Chaque section est autonome. Le README est mis à jour pour linker vers ce fichier. Le skill `implement-ticket` est mis à jour pour maintenir cette doc à jour.

**Tech Stack:** Markdown, cross-référencement avec `config.example.yaml`, `board.example.yaml`, `README.md`, `src/main.ts`

---

### Task 1 : Squelette + Section 1 — Pré-requis

**Files:**
- Create: `docs/configuration.md`

- [ ] **Créer le fichier avec le squelette complet et la Section 1**

Créer `docs/configuration.md` avec le contenu suivant :
...
**Lien rapide :** si tu es déjà configuré → [Référence complète](#référence-complète)
...
- [ ] **Vérifier** que les URLs Jira dans les explications sont exactes (tester mentalement contre une URL connue type `https://company.atlassian.net/jira/software/projects/PROJ/boards/42`)

- [ ] **Commit**

```bash
git add docs/configuration.md
git commit -m "docs(config): add configuration guide skeleton and section 1 prerequisites"
```

---

### Task 2 : Section 2 — `config.yaml`

**Files:**
- Modify: `docs/configuration.md`
- Reference: `config.example.yaml`

- [ ] **Ajouter la Section 2 à la fin du fichier**
...
- [ ] **Cross-vérifier** contre `config.example.yaml` : tous les champs présents dans l'exemple sont couverts dans au moins un des 3 blocs

- [ ] **Commit**

```bash
git add docs/configuration.md
git commit -m "docs(config): add section 2 config.yaml with 3 auth blocs"
```

---

### Task 3 : Section 3a — `board.yaml` via `autoconfig`

**Files:**
- Modify: `docs/configuration.md`
- Reference: `board.example.yaml`, `README.md` (section autoconfig)

- [ ] **Ajouter la Section 3 (intro + 3a) à la fin du fichier**
...
- Détecte les colonnes "queue" par mots-clés : review, validation, QA, attente, staging, approval…
...
- [ ] **Vérifier** que la liste des mots-clés `autoconfig` (review, validation, QA…) correspond à ce qui est dans `src/main.ts` — chercher `inferBoardColumns` pour la liste exacte

- [ ] **Commit**

```bash
git add docs/configuration.md
git commit -m "docs(config): add section 3a board.yaml via autoconfig"
```

---

### Task 4 : Section 3b+3c — `board.yaml` manuel + cas particuliers

**Files:**
- Modify: `docs/configuration.md`
- Reference: `board.example.yaml`

- [ ] **Ajouter les sections 3b et 3c**
...
- [ ] **Cross-vérifier** le contenu `board.example.yaml` : tous les champs mentionnés dans 3b sont présents dans l'exemple ou dans le texte

- [ ] **Commit**

```bash
git add docs/configuration.md
git commit -m "docs(config): add section 3b manual board config and 3c edge cases"
```

---

### Task 5 : Section 4 — Valider et premier lancement
...
Lance un premier `sync` si la base est vide, puis vérifie que chaque statut de `board.yaml` existe bien dans Jira.
...
> Si la sortie liste un statut inconnu : vérifier l'orthographe exacte dans `board.yaml` (la casse compte).
...
Ouvrir `report.html` dans un navigateur. Si les métriques semblent fausses :
...
- [ ] **Vérifier** que la sortie `validate` dans la doc correspond au format réel — checker `src/main.ts` pour la commande `validate-config`

- [ ] **Commit**

```bash
git add docs/configuration.md
git commit -m "docs(config): add section 4 validate and first launch sequence"
```

---

### Task 6 : Section 5 — Référence complète

**Files:**
- Modify: `docs/configuration.md`
- Reference: `config.example.yaml`, `board.example.yaml`, `src/main.ts`

- [ ] **Ajouter la Section 5**
...
| `personalAccessToken` | string | Non* | — | PAT Jira Server/DC. *Requis pour auth PAT. Prend le dessus sur `email`/`apiToken` si présent et non vide |
...
|---|---|---|---|
| `name` | string | Oui | Nom de la colonne (affiché dans les logs, pas dans les métriques) |
...
- [ ] **Cross-vérifier chaque champ** contre `src/main.ts` (fonctions `loadJiraConfig`, `loadBoardConfig`) pour s'assurer qu'aucun champ n'est oublié ni mal documenté

- [ ] **Commit**

```bash
git add docs/configuration.md
git commit -m "docs(config): add section 5 complete field reference"
```

---

### Task 7 : Sections 6 et 7 — Cas avancés + Troubleshooting

**Files:**
- Modify: `docs/configuration.md`

- [ ] **Ajouter les Sections 6 et 7**
...
**Cause probable :** bulk-close lors d'une migration de workflow Jira (des centaines de tickets fermés en masse à une date précise).
...
```bash
git add docs/configuration.md
git commit -m "docs(config): add sections 6 advanced use cases and 7 troubleshooting"
```

---

### Task 8 : Lien README → `docs/configuration.md`

**Files:**
- Modify: `README.md`

- [ ] **Ajouter un lien dans README.md**

Dans `README.md`, après la section `## Configuration` (ligne ~38), ajouter juste après le tableau des deux fichiers :
...
- [ ] **Vérifier** que le lien Markdown pointe bien vers `docs/configuration.md` (chemin relatif depuis la racine du repo)

- [ ] **Commit**

```bash
git add README.md
git commit -m "docs: link to configuration guide from README"
```

---

### Task 9 : Mettre à jour le skill `implement-ticket`

**Files:**
- Modify: `.claude/skills/implement-ticket/SKILL.md`

Ce task ajoute `docs/configuration.md` à la liste des docs à maintenir dans la **Definition of Done** et la **Phase 7** du skill `implement-ticket`, conformément à la contrainte de maintenance définie dans le design spec.

- [ ] **Modifier la Definition of Done**

Dans `.claude/skills/implement-ticket/SKILL.md`, dans la section `## Definition of Done`, ajouter après la ligne `**Si surface publique modifiée** ... README.md mis à jour` :
...
Dans la section `### Phase 7 — Mise à jour specs système + CLAUDE.md + README.md (conditionnelle)`, ajouter dans le paragraphe `**Procédure**` :
...
- [ ] **Modifier le titre de la Phase 7** pour inclure `docs/configuration.md` :
...
```bash
git add .claude/skills/implement-ticket/SKILL.md
git commit -m "docs(skill): add docs/configuration.md to implement-ticket maintenance checklist"
```
...
**Couverture spec :**
- ✓ Section 1 — Pré-requis (Task 1)
- ✓ Section 2 — config.yaml 3 blocs (Task 2)
- ✓ Section 3a — autoconfig (Task 3)
- ✓ Section 3b — manuel + types colonnes + role (Task 4)
- ✓ Section 3c — legacyDoneStatuses + cutoffDate (Task 4)
- ✓ Section 4 — validate + sync + refresh (Task 5)
- ✓ Section 5 — référence champ par champ (Task 6)
- ✓ Section 6 — multi-squad + personnalisation + mode fake (Task 7)
- ✓ Section 7 — troubleshooting 6 cas (Task 7)
- ✓ Lien README (Task 8)
- ✓ Contrainte maintenance implement-ticket (Task 9)

**Vérifications transverses :**
- Chaque tâche cross-référence les fichiers source pour vérifier l'exactitude
- Pas de placeholder ni TBD
- Contenu complet dans chaque step — pas de "voir section X" sans contenu autonome
