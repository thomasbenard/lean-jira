...
3. `GET /rest/agile/1.0/board/{boardId}/issue?expand=changelog&fields=…` → toutes les issues (pagination 100/page, 200 ms entre pages).
4. Mappe chaque issue : `current_sprint_id` = sprint actif courant uniquement (ignore les sprints fermés historiques).
5. Upsert `issues` + `sprints` en transaction.
6. Pour chaque issue : `replaceTransitions` — DELETE + INSERT atomique dans `transitions`. Garantit cohérence si Jira modifie l'historique.
