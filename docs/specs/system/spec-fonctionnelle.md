...
2. **Tendances hebdomadaires** : 10 graphiques (lead time, cycle time, throughput, throughput pondéré, WIP, bugs, bug cycle time, cycle normalisé, flow efficiency, allocation dev features/bugs). Chaque graphique affiche une courbe de tendance superposée calculée par moyenne mobile sur une fenêtre de 4 semaines (gris ardoise `#64748b88`, pointillé `[6,4]`, label "Tendance"). Les 3 premiers points ne sont pas tracés (fenêtre insuffisante). La courbe porte sur la série principale (médiane pour les métriques de durée, `count`/`estimatedDays` pour les métriques de débit).
