...
describe("bugCycleTimeMetric.compute", () => {
  it("retourne stats vides si bugIssueTypes est vide", () => {
    ...
    const result = bugCycleTimeMetric.compute(createTestContext(db, { ...TEST_CONFIG, bugIssueTypes: [] }));
    expect(result.count).toBe(0);
  });

  it("retourne stats vides si aucun bug livré", () => {
    const result = bugCycleTimeMetric.compute(createTestContext(db, TEST_CONFIG));
    expect(result.count).toBe(0);
  });
  ...
  it("cutoffDate respecté", () => {
    ...
    const result = bugCycleTimeMetric.compute(createTestContext(db, { ...TEST_CONFIG, cutoffDate: "2025-01-09" }));
    expect(result.count).toBe(0);
  });

  it("windowEndDate respecté", () => {
    seedBug("BUG-1", "2025-01-06T09:00:00Z", "2025-01-10T09:00:00Z"); // done jan 10
    const result = bugCycleTimeMetric.compute(createTestContext(db, { ...TEST_CONFIG, windowEndDate: "2025-01-09" }));
    expect(result.count).toBe(0);
  });
});
