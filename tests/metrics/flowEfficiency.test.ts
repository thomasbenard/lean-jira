...
describe("flowEfficiencyMetric.compute", () => {
  it("retourne résultat vide si activeStatuses est vide", () => {
    ...
    const result = flowEfficiencyMetric.compute(db, cfg);
    expect(result.count).toBe(0);
    expect(result.issues).toHaveLength(0);
  });

  it("retourne résultat vide si aucune issue dans la population cycle-time", () => {
    const result = flowEfficiencyMetric.compute(db, TEST_CONFIG);
    expect(result.count).toBe(0);
  });
  ...
  it("cutoffDate respecté", () => {
    ...
    const result = flowEfficiencyMetric.compute(db, { ...TEST_CONFIG, cutoffDate: "2025-01-14" });
    expect(result.count).toBe(0);
  });

  it("windowEndDate respecté", () => {
    seedFlowIssue(); // done 2025-01-13
    const result = flowEfficiencyMetric.compute(db, { ...TEST_CONFIG, windowEndDate: "2025-01-12" });
    expect(result.count).toBe(0);
  });
});
