...
describe("throughputWeighted — méthode time", () => {
  it("retourne unit='j-h' et disabled=false", () => {
    const result = throughputWeightedMetric.compute(createTestContext(db, cfg("time")));
...
describe("throughputWeighted — méthode story-points", () => {
  it("unit='SP' et somme story_points", () => {
    ...
    const result = throughputWeightedMetric.compute(createTestContext(db, cfg("story-points")));
...
describe("throughputWeighted — méthode numeric", () => {
  it("unit='pts' et somme story_points", () => {
    ...
    const result = throughputWeightedMetric.compute(createTestContext(db, cfg("numeric", { jiraField: "customfield_10099", bucketThresholds: { xs: 2, s: 5, m: 10, l: 20 } })));
...
describe("throughputWeighted — méthode t-shirt", () => {
  it("retourne disabled=true", () => {
    const result = throughputWeightedMetric.compute(createTestContext(db, cfg("t-shirt", { jiraField: "customfield_10200" })));
...
describe("throughputWeighted — méthode none", () => {
  it("retourne disabled=true", () => {
    const result = throughputWeightedMetric.compute(createTestContext(db, cfg("none")));
    expect(result.disabled).toBe(true);
    expect(result.byWeek).toHaveLength(0);
  });
});
