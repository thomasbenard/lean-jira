...
describe("leadTimeMetric.compute", () => {
  it("retourne stats vides quand aucune issue livrée", () => {
    const result = leadTimeMetric.compute(createTestContext(db, TEST_CONFIG));
  ...
  it("cutoffDate exclut les issues livrées avant", () => {
    ...
    const result = leadTimeMetric.compute(createTestContext(db, { ...TEST_CONFIG, cutoffDate: "2025-01-11" }));
    expect(result.count).toBe(0);
  });

  it("windowEndDate exclut les issues livrées après", () => {
    seedCanonical(); // done 2025-01-10
    const result = leadTimeMetric.compute(createTestContext(db, { ...TEST_CONFIG, windowEndDate: "2025-01-09" }));
    expect(result.count).toBe(0);
  });
  ...
  it("excludeIssueTypes exclut les issues Feature et Epic", () => {
    ...
    const result = leadTimeMetric.compute(createTestContext(db, { ...TEST_CONFIG, excludeIssueTypes: ["Feature", "Epic"] }));
    expect(result.count).toBe(1);
